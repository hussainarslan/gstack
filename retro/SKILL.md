...
# 12. sage skill usage telemetry (if available)
cat ~/.sage/analytics/skill-usage.jsonl 2>/dev/null || true
...
**Eureka Moments (if logged):** Read `~/.sage/analytics/eureka.jsonl` if it exists. Filter entries within the retro time window by `ts` field. For each eureka moment, show the skill that flagged it, the branch, and a one-line summary of the insight. Present as:
...
```bash
DISCOVER_BIN=""
[ -x ~/.claude/skills/sage/bin/gstack-global-discover ] && DISCOVER_BIN=~/.claude/skills/sage/bin/gstack-global-discover
[ -z "$DISCOVER_BIN" ] && [ -x .claude/skills/sage/bin/gstack-global-discover ] && DISCOVER_BIN=.claude/skills/sage/bin/gstack-global-discover
[ -z "$DISCOVER_BIN" ] && which gstack-global-discover >/dev/null 2>&1 && DISCOVER_BIN=$(which gstack-global-discover)
[ -z "$DISCOVER_BIN" ] && [ -f bin/gstack-global-discover.ts ] && DISCOVER_BIN="bun run bin/gstack-global-discover.ts"
echo "DISCOVER_BIN: $DISCOVER_BIN"
```

If no binary is found, tell the user: "Discovery script not found. Run `bun run build` in the sage directory to compile it." and stop.
...
║
║  Powered by sage · github.com/hussainarslan/gstack
╚═══════════════════════════════════════════════════════════════
```
...
```bash
ls -t ~/.sage/retros/global-*.json 2>/dev/null | head -5
```
...
```bash
today=$(date +%Y-%m-%d)
existing=$(ls ~/.sage/retros/global-${today}-*.json 2>/dev/null | wc -l | tr -d ' ')
next=$((existing + 1))
```

Use the Write tool to save JSON to `~/.sage/retros/global-${today}-${next}.json`:
...
```json
{
  ...
  "tweetable": "Week of Mar 14: 5 projects, 182 commits, 15.3k LOC | CC: 48, Codex: 8, Gemini: 3 | Focus: sage (58%) | Streak: 52d"
}
```
...
- **Global mode:** Does NOT require being inside a git repo. Saves snapshots to `~/.sage/retros/` (not `.context/retros/`). Gracefully skip AI tools that aren't installed. Only compare against prior global retros with the same window value. If streak hits 365d cap, display as "365+ days".
